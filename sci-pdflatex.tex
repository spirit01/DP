...
\end{lstlisting}
byl získán soubour NAME.47, který sloužil jako vstup do programu NBO6.0. Přidáním klíčového slova $PLOT$ do souboru NAME.47 a použitím programu NBO6.0 byly ziskány soubory NAME.31 až NAME.46. Pro vykreslení přirozených orbitalů programem NBO6View byl použit soubor NAME.37 \cite{doi:10.1002/jcc.23266}.
...
% Mezi malé modely jsme v této studii zařadili následující reaktanty použité v experimentální práce Aleše Stýskalíka: \ce{Si(OC2H5)4}, \ce{SiF3(C6H5)}, \ce{SiCH3(OCOCH3)3}, \ce{Si(OCOCH3)4} a \ce{Si(OCl)(OC2H5)3}. Struktury jsou uvedeny na obrázku
%Dále jsme provedli výpočty na modelech zjednodušených vůči reaktantům náhradou esterických substituentů methoxy skupina: \ce{SiCH3(OCH3)3}, \ce{Si(OCH3)4}. Pro modelování šestikoordinovaného křemíku v silikofosfátech byly ještě zařazeny struktury \ce{SiCH3(OCOCH3)3} a \ce{Si(OCOCH3)4}.
%První část tohto odstavce se věnuje zjednodušeným modelům reaktantů. Druhá část se věnuje reaktantům, které slouží pro experimentální přípravu silikofosfátových polymerů.
 Struktury zkoumané v první části jsou uvedeny na obrázku \ref{prehled_male_modely}. V silikofosfátocých polymerech se vyskytuje křemík v koordinaci čtyři a šesta s vazbami na kyslíky. Model \ce{SiCH3(OCH3)3} \ref{prehled_male_modely}, \textbf{A} představuje strukturu s přímou vazbou Si-C. Toto uspořádání brání křemíku navyšovat koordinaci. Právě ochota křemíku navyšovat koordinaci je jednou z klíčových otázek této práce. Model \ce{Si(OCH3)4} \ref{prehled_male_modely}, \textbf{B} představuje uspořádání křemíku v koordinaci čyři ve kterém je křemík ochoten koordinaci navyšovat. Naopak v modelech \ce{SiCH3(OCH3)5^{2-}} \ref{prehled_male_modely}, \textbf{C} a \ce{Si(OCH3)6^{2-}} \ref{prehled_male_modely}, \textbf{D} se křemmík vyskytuje v koordinaci šest. Při optimalizace geometrie struktury  \ce{SiCH3(OCH3)5^{2-}} \ref{prehled_male_modely}, \textbf{C} bylo nutno použit klíčové slovo Opt=CalcFC a při optimalizace struktury \ce{Si(OCH3)6^{2-}} \ref{prehled_male_modely}, \textbf{D} bylo použito klíčové slovo SCF=Vshift. Poslední dvě strutkury  \ref{prehled_male_modely}, \ce{SiCl4},  \textbf{E} a (acetylmethoxyl)trifluorsilan, \textbf{F}  obsahují křemík, který má jako své ligandy elektronegativní atomy. Model \ce{SiCl4} slouží jako případ sloučeniny, kde křemík není schopen navyšovat koordinaci. Naopak (acetoxymethyl)trifluorosilan je schopen tvořit intramolekulární vazbu s Si-O a tím navýšit koordinaci křemíku na pět. Tyto  struktury jsou experimentálně pozorovány a nazývají se dragonoidy \cite{Chipanina2011}.
...
Druhá část této kapitoly je věnována reaktantům, které byly použity v experimentální práci Aleše Stýskalíka pro přípravu silikofosfátových polymerů.
\begin{figure}
\centering
...
\subfigure[\ce{Si(OCl)(OC2H5)3},  \textbf{I}]{\includegraphics[width=5cm]{si_ocl_oet_3.png}\label{si_ocl_oet_3}}
...
\begin{figure}
\centering
\subfigure[\ce{Si(OCOCH3)4},  \textbf{J}]{\includegraphics[width=5cm]{si_o_ac_4.png}\label{si_o_ac_4}}
\subfigure[\ce{SiH(OCOCH3)3},  \textbf{K}]{\includegraphics[width=5cm]{si_h_oac_3.png}\label{si_h_oac_3}}
\subfigure[\ce{SiCH3(OCOCH3)3},  \textbf{L}]{\includegraphics[width=5cm]{si_ch3_oac_3.png}\label{si_ch3_oac_3}}
\caption{Přehled reaktantů; Legenda: \mycircle{brown} Si, \mycircle{red} O, \mycircle{orange} C, \mycircle{red!50!blue!50} F, \mycircle{green}~Cl, \mycircleempty{black}~H}
\label{prehled_male_modely_II}
\end{figure}


Jedno z kritérii pro stabilitu molekul je rozdíl energie orbitalů HOMO a LUMO. Kladná hodnota energie orbitalu HOMO může znamenat méně nestabilní sloučeninu a naopak. Obdobně lze použít i parametr $\eta$, kdy vyšší hodnota $\eta$ znamená vyšší stabilitu sloučiny podle teorie HSAB.

Tyto dvě uspořádání modelují struktury \ref{prehled_male_modely}, \textbf{B} a \textbf{D}. Model \ref{prehled_male_modely}, \textbf{A} představuje sloučeninu, která obsahuje přímou vazmu Si-C, která v silikofosfátech není pozorována. Toto uspořádání brání křemíku navyšovat svoji koordinaci a podle hodnoty $\eta=$ 2,831 se jedná o relativně stabilní molekulu. Oproti tomu struktura  \ref{prehled_male_modely}, \textbf{C} není pozorována a tomu odpovídá i velice nízká hodnota $\eta=$ -6,589. Struktura  \textbf{C} je tedy nestabilní a pravděpodobně nemůže existovat.

 Naopak (acetylmethoxyl)trifluorsilan \ref{prehled_male_modely}, \textbf{F} je sloučenina, kde je křemík ochoten zvyšovat koordinaci na pět.
...
Pro všechny malé modely byla provedena NBO analýza. Ta si kladla za cíl lépe popsat vazebné možosti křemíku v různých koordinacích a s různými substituenty. V NBO analýze byly analyzovány protivazebné orbitaly (BD*). Právě dostupnost BD* určuje, zda bude sloučenina křemíku ochotna navyšovat koordinaci. Na základě vysokého podílu křemíku v BD* u molekuly \textbf{A} v tabulce \ref{nbo_small} lze učit, v případě vazebných orbitalů (BD) je velká část vazby lokalizována na kyslíku.
...
Pro některé molekuly bylo nutno použít vice MO než je počet jejich vazeb kvůli dalokalizaci MO. Zapojení orbitalů typu $p$ je dáno přibližně 4-četnou osou symetrie. Proto byly pro MPA analýzy byly vybrány takové vazebné orbitaly, ve kterých podíl křemíku a alespoň jednoho z přímo vázaných ligandů alespoň 0,1. Vzhledem k charakteru MPA analýzy nebylo možné použít pro analýzy orbitaly LUMO.
...
Na základě experimentální práce Aleše Stýskalíka \cite{Styskalik2015thesis} jsem se zabývala křemíkem v koordinaci čtyři, který sloužil jako výchozí reaktant pro přípravu silikofosfátových polymerů. První část tvoří reaktanty, které vedou ke vzniku silikofosfátových polymerů v bezvodém prostředí. Druhou skupinou je speciální případ \ce{Si(OCOCH3)4} a jeho modifikací. Zde byla klíčová otázka, kdy je křemík ochoten navýšit koordinaci na šest. Výchozím modelem byl \ce{Si(OCOCH3)4},\ref{prehled_male_modely_II}, \textbf{J} a potom dvě jeho modifikované struktury. V prvním případě byl jeden acetát nahrazen methylem  \ref{prehled_male_modely_II}, \textbf{K}, v druhém případě byl acetát nahrazen přímo vodíkem \ref{prehled_male_modely_II}, \textbf{K}. Pro obě skupiny byly zpracována HSAB analýzy.
...
Podrobnější pohled na chemickkou vazbu dávají obrázky NBO orbitalů.
