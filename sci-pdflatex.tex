...
abstract      = {Práce je zaměřená na studium silikofosfátů a reaktantů používaných v jejich přípravě. Studované struktury jsou rozděleny podle velikosti na tři části. Malé modely reprezentují blízké okolí křemíku v silikofosfátech a zároveň jsou sem zahrnuty i reaktanty, které byly používány v experimentálních pracích. Střední modely již obsahují cykly a velké modely reprezentují výsek ze skutečných silikofosfátových polymerů. Strukutury jsou studovány na úrovni teorie funkcionálu hustoty. Pro porovnání struktur je použita teorie tvrdých a měkkých kyselin a bází. Pro pochopení vazebných možností křemíku je použita teorie přirozených orbitalů a mullikenova populační analýza. Pro velké struktury jsou vypočítány hodnoty chemických posunů a jsou porovnány s experimentálnimi hodnotami nmr posunů.

Z experimentálních prací vyplývá, že v přítomnosti některých ligandů křemík ztrácí schopnost navyšovat svoji koordinaci. V tomto případě byly na úrovni HSAB porovnány struktury, které vedou k navyšování koordinace a struktury, u kterých navyšování kordinace není pozorováno. Druhý parametr porovnání je na základě molekulových orbitalů. \\
...
\begin{table}[H] \begin{minipage}{\textwidth}
\begin{center}
\caption{MPA analýza vybraných MO orbitalů \ce{SiCl4}, \textbf{E}, výsledky jsou uvedeny v~[\%].}
\begin{tabular}{|l|c|c|c|c|c|}
\hline
...
\begin{table}[H] \begin{minipage}{\textwidth}
\caption{MPA analýza vybraných MO orbitalů acetylmethyltrifluorsilanu, \textbf{F}, výsledky jsou uvedeny v~[\%].}
\begin{center}
\begin{tabular}{|l|c|c|c|c|c|}
\hline
...
\begin{figure}\begin{center}\includegraphics[width=6cm]{schema_fosfat_1.eps}
...
\begin{figure}\begin{center}\includegraphics[width=6cm]{struktrua_c_trans_1.eps}
...
\begin{tabular}{|l|c|c|c|c|c|c|c|}
\hline
\label{nbo_small} &  Vazba & O.Č\footnote{Obsazovací číslo orbitalu.} & Si\footnote{Procentuální příspěvěk křemíku do NBO.} & X\footnote{Procentuální příspěvek atomu vázaného na křemíku do NBO.} & Si(s)\footnote{Rozdělení příspěvku křemíku do orbitálních komponent, součet Si(s) + Si(p) + Si(d) = 100\%.} & Si(p)$^d$ &Si(d)$^d$ \\ \hline
