...
\begin{figure}
\caption{Struktura s 12-členným kruhem nakreslená autorkou DP v programu Avogadro na základě dat uvedených v článku \cite{velky_cyklus}.  Struktura byla upravena tak, aby byla viditelná existence 12-členného cyklu, neboť v původní prácei chybí konektivita jednoho z atomů fosforu do cyklu.;  Legenda: \mycircle{brown} Si, \mycircle{red}~O, \mycircle{orange}~C, \mycircle{yellow} P, \mycircleempty{black}~H.}
...
\end{center}
\caption{Koordinační okolí křemíku v silikofosfátech \cite{rtg_4_pinkas}.}
\end{figure}
...
\end{equation}
kde $\sigma$ je tzv. stínící konstanta (většinou kladá, někdy však záporná). Jádra v různých chemických skupinách mají odlišné stínící konstanty  a to díky rozdílnému rozložení elektronů. \cite{atkins2010atkins}. Výpočet stínící konstanty vyžaduje podrobné informace o elektronové hustotě a lze ji rozdělit na tři příspěvky: lokální, okolních skupin a rozpouštědla.

Pro lokální pole platí, že:
\begin{equation}
\vec{B_{loc}} = \vec{B_0} + \delta \vec{B_0}
\end{equation}
$\vec{B_{loc}}$ je lokální magnetické pole. $\delta$ je tzv. chemický posun. Ten může být jiný pro stejné prvky s jiným koordinančím okolí a je empirickou veličinou. Chemický posun je definován jako rozdíl mezi rezonanční frekvencí jádra a rezonanční frekvencí standardu.
...
Pro porovnání stability malých modelů viz. obrázek \ref{prehled_male_modely} byla použita teorie HSAB. Jedno z kritérii pro stabilitu sloučenin je parametr $\eta$, kdy vyšší hodnota $\eta$ znamená vyšší stabilitu sloučiny. Druhým parametrem je $\chi$, který popisuje ochotu moekuly přijítmat další elektronový pár.

Z výsledků v tabulce \ref{hsab_small} lze vidět, že struktura \ce{Si(OCH3)4}, \textbf{B} má vyšší hodnotu $\eta$ než \ce{SiCH3(OCH3)3}, \textbf{A} a díky tomu je stabilnější. Na druhou stranu je hodnota $\chi$ zrcadlová, tedy molekula \textbf{A} má nižší hodnotu $\chi$. U molekula \textbf{A} tedy vzrostla kovalence vazby oproti \textbf{B}.

 Stabilita dvojice \ce{Si(OCH3)6^{2-}} a \ce{SiCH3(OCH3)5^{2-}} klesla. Hodnota energie LUMO orbitalu pro tyto dva modely není úplně správná a to díky zápornému náboji, který zde nebyl nijak kompenzován.
...
Na základě experimentální práce Aleše Stýskalíka \cite{Styskalik2015thesis} jsem se zabývala křemíkem v koordinaci čtyři, který sloužil jako výchozí reaktant pro přípravu silikofosfátových polymerů. Struktury reaktantů jsou uvedeny na obrázku \ref{prehled_male_modely_II}. Na základě výskytu methoxy skupiny v malých modelech jsme zkusili nahradit methoxy skupinu za ethoxy skupinu. V případě této záměny elektronegativity očekávaně klesla na hodnotu 2,760 eV.
%\begin{figure}
%\centering
...
První část tvoří reaktanty, které vedou ke vzniku silikofosfátových polymerů v bezvodém prostředí. Jednou z otázek zodpovězených v této práci je i okolí křemíku, které vede ke vzniku šestikoordinovaného křemíku. Druhou skupinou byl proto speciální případ \ce{Si(OCOCH3)4} a dvou jeho modifikací. Zde byla klíčová otázka, kdy je křemík ochoten navýšit koordinaci na šest. Výchozím modelem byl \ce{Si(OCOCH3)4} viz. obrázek \ref{prehled_male_modely_II}, \textbf{J} a potom dvě jeho modifikované struktury. V prvním případě byl jeden acetát nahrazen methylem \ce{SiH(OCOCH3)3}  viz. obrázek \ref{prehled_male_modely_II},\textbf{K}, v druhém případě byl acetát nahrazen přímo vodíkem \ce{SiCH3(OCOCH3)3}viz. obrázek  \ref{prehled_male_modely_II}, \textbf{L}. \\
%\textbf{G} & -7,181 & 1,662 & 2,760 & 4,422 \\ \hline

HSAB analýza pro porovnání struktury \ce{Si(OCOCH3)4} \textbf{J} a jejími modifikacemi v tabulce \ref{hsab_small_porovnani}ukazuje, že náhradou acetátu za vodík ve struktuře \textbf{K} a následnou náhradou za methyl ve struktuře \textbf{L} vede k nárustu tvrdosti a tedy menšímu sklonu dále reagovat.
...
V modelu \textbf{O} je křemík obklopen šesti fosfáty, které jsou vzájemně zesíťované přes další čtyřkoordinované křemíky. Přítomnost osmičlenného cyklu v silikofosfátech byla potvrzena rentgenovou strukturou \ref{rtg_4} ve článku \cite{rtg_4_pinkas}. Sloužila jako motiv pro vytváření cyklů v silikofofátových polymerech.
\begin{figure}
\begin{center}
